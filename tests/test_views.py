...
class AdUpdateViewTest(TestCase):
    ...
    def test_ad_update_view_redirects_on_success(self):
        """Тест проверяет, что после редактирования объявления, происходит перенаправление
        на страницу с моими объявлениями."""
...
class AdDeleteViewTest(TestCase):
    ...
    def test_ad_delete_view_redirects_on_success(self):
        """Тест проверяет, что после удаления объявления, происходит перенаправление
        на страницу с моими объявлениями."""
...
class MyExchangeProposalListViewTest(TestCase):
    ...
    def setUp(self):
        ...
        self.exchange_proposed = ExchangeProposal.objects.create(
            owner=self.user, ad_sender=self.ad1, ad_receiver=self.ad2, status="Ожидает"
        )
        self.other_exchange = ExchangeProposal.objects.create(
            owner=self.user, ad_sender=self.ad2, ad_receiver=self.ad1, status="Подтвержден"
        )
...
class OffersExchangeProposalListViewTest(TestCase):
    ...
    def setUp(self):
        ...
        self.exchange_proposed = ExchangeProposal.objects.create(
            owner=self.user, ad_sender=self.ad1, ad_receiver=another_ad, status="Ожидает"
        )

        self.exchange_from_other = ExchangeProposal.objects.create(
            owner=other_user, ad_sender=another_ad, ad_receiver=self.ad1, status="Ожидает"
        )
...
class ExchangeProposalDeleteViewTest(TestCase):
    ...
    def test_exchange_proposal_delete_view_redirects_on_success(self):
        """Тест проверяет, что после удаления предложения происходит перенаправляется
        на страницу с моими предложениями обмена."""
...
class AdSearchListViewTest(TestCase):
    ...
    def setUp(self):
        ...
        self.ad1 = Ad.objects.create(
            title="Тест 1", description="Описание 1", category="одежда", condition="новый", user=self.user
        )
        self.ad2 = Ad.objects.create(
            title="Тест 2", description="Описание 2", category="обувь", condition="б/у", user=self.user
        )
        self.another_user = User.objects.create_user(email="another@user.mail.ru", password="another_pass")
        self.another_ad = Ad.objects.create(
            title="Тест 3", description="Описание 3", category="одежда", condition="новый", user=self.another_user
        )
